...
-- Initial Functions --
-----------------------
laboratory.medium_mixer = {}

local medium_mixer = laboratory.medium_mixer

medium_mixer.recipes = {}

function medium_mixer.register_recipe(input, output)
    medium_mixer.recipes[input] = output
end
...
local medium_mixer_fs = "formspec_version[3]" .. "size[12.75,8.5]" ..
...
local function get_active_medium_mixer_fs(item_percent)
    local form = {
...
local function update_formspec(progress, goal, meta)
    ...
    if progress > 0 and progress <= goal then
        ...
        formspec = get_active_medium_mixer_fs(item_percent)
    else
        formspec = medium_mixer_fs
    end
...
local function cultivate(pos)
    ...
    local output_item = medium_mixer.recipes[input_item:get_name()]
    input_item:set_count(1)

    if not medium_mixer.recipes[input_item:get_name()] or
...
minetest.register_node("hades_laboratory:medium_mixer", {
    ...
    on_timer = function(pos)
        ...
        if not medium_mixer.recipes[stack:get_name()] then return false end
        ...
        local output_item = medium_mixer.recipes[stack:get_name()]
        local output_time = 33;
    ...
    allow_metadata_inventory_put = function(pos, listname, _, stack, player)
        ...
        if listname == "input" then
            return medium_mixer.recipes[stack:get_name()] and
    ...
    on_metadata_inventory_put = function(pos)
        ...
        local output_item = medium_mixer.recipes[stack:get_name()]
        local cultivating_time = meta:get_int("cultivating_time") or 0
        if not medium_mixer.recipes[stack:get_name()] then
            timer:stop()
    ...
    on_blast = function(pos)
        ...
        table.insert(drops, "hades_laboratory:medium_mixer")
