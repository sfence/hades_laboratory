...
laboratory.polymerase_extractor = {}

local polymerase_extractor = laboratory.polymerase_extractor

polymerase_extractor.recipes = {}

function polymerase_extractor.register_recipe(input, output)
    polymerase_extractor.recipes[input] = output;
end
...
local polymerase_extractor_fs = "formspec_version[3]" .. "size[12.75,8.5]" ..
...
local function get_active_polymerase_extractor_fs(item_percent)
    local form = {
...
local function update_formspec(progress, goal, meta)
    ...
    if progress > 0 and progress <= goal then
        ...
        formspec = get_active_polymerase_extractor_fs(item_percent)
    else
        formspec = polymerase_extractor_fs
    end
...
local function cultivate(pos)
    ...
    local output_item = polymerase_extractor.recipes[input_item:get_name()]
    input_item:set_count(1)

    if not polymerase_extractor.recipes[input_item:get_name()] or
...
minetest.register_node("hades_laboratory:polymerase_extractor", {
    ...
    on_timer = function(pos)
        ...
        if not polymerase_extractor.recipes[stack:get_name()] then return false end
        ...
        local recipe = polymerase_extractor.recipes[stack:get_name()]
    ...
    allow_metadata_inventory_put = function(pos, listname, _, stack, player)
        ...
        if listname == "input" then
            return polymerase_extractor.recipes[stack:get_name()] and
    ...
    on_metadata_inventory_put = function(pos)
        ...
        local stack = inv:get_stack("input", 1)
        local filters_in = inv:get_stack("filters_in", 1)
        local output_item = polymerase_extractor.recipes[stack:get_name()]
        local cultivating_time = meta:get_int("cultivating_time") or 0
        if not polymerase_extractor.recipes[stack:get_name()] then
            timer:stop()
    ...
    on_blast = function(pos)
        ...
        table.insert(drops, "hades_laboratory:polymerase_extractor")
...
if laboratory.have_paleotest then
  polymerase_extractor.register_recipe( "hades_laboratory:bottle_of_bacteries",
